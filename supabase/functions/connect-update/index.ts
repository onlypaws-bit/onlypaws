...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ⚠️ TEMP hardcode (poi lo leghiamo al DB per-user)
    ...
    console.log("connect account status", {
      ...
      disabled_reason: acc.requirements?.disabled_reason,
    ...
    // ✅ For this account Stripe only allows account_onboarding
    ...
    return json(200, { url: link.url, type: "account_onboarding" });
  } catch (e) {
    console.error("CONNECT UPDATE ERROR:", e);
    const msg =
      (e as any)?.raw?.message ||
      (e as any)?.message ||
      String(e);
